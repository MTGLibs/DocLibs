...
public class WPControl extends AbstractControl {
                ...
                if (link != null) {
                    try {
                        if (link.getLinkType() == Hyperlink.LINK_BOOKMARK) {
                            ...
                            if (bm != null) {
                                getMainFrame().onEventMethod(null, null, null, -1.0f, -1.0f, IMainFrame.ON_HYPER_LINK_BOOK_MARK);
                                ControlKit.instance().gotoOffset(wpView, bm.getStart());
                            }
                        } else {
                            Intent intent = new Intent(Intent.ACTION_VIEW, Uri.parse(link.getAddress()));
                            getMainFrame().onEventMethod(null, null, null, -1.0f, -1.0f, IMainFrame.ON_HYPER_LINK_URL);
    ...
    public Object getActionValue(int actionID, Object obj) {
        switch (actionID) {
            ...
            case EventConstant.APP_ZOOM_MAX_ID:
                return wpView.getFitZoom() * WPViewConstant.PAGE_MAX_ZOOM;
//            case EventConstant.APP_SCROLL_PERCENT_Y:
//                return wpView.getScrollY();
    ...
    public void showPage(int index, int direction) {
        wpView.showPage(index, direction);
    }
