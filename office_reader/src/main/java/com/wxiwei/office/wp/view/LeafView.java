...
import com.wxiwei.office.wp.model.CharText;
...
//        paint.setColor(Color.rgb(random.nextInt(254),random.nextInt(254),random.nextInt(254)));
...
//        }
    ...
    private SoftReference<ArrayList<CharText>> charList = new SoftReference<>(null);
...
//        CharacterRun cr = (CharacterRun)elem;
        ...
        if (charAttr.subSuperScriptType == 1) {
            dY -= Math.ceil((paint.descent() - paint.ascent()));
        }
        // 下标
        ArrayList<CharText> list = charList.get();
        if (list != null) {
            if (list.size() > 0) {
                float scale = zoom / list.get(0).zoom;
                for (CharText charText : list) {
                    canvas.drawText(charText.text, dX+charText.x * scale, dY+charText.y * scale, paint);
                }
            }
        } else {
            list = new ArrayList<>();
            ...
            if (getControl().getView() instanceof Word) {
                canvas.drawText(text, s, e, drawX, drawY, paint);
                CharText charText = new CharText(text.substring(s, e), drawX - dX, drawY - dY, zoom);
                list.add(charText);
            }else {
                for (int i = s; i < e; i++) {
                    ...
                    CharText charText = new CharText(text.substring(i, i + 1 + skip), drawX - dX, drawY - dY, zoom);
                    list.add(charText);
                    drawX += widths[i] - extX;
                    i += skip;
            ...
            charList = new SoftReference<>(list);
        }

        // 绘制删除线
        dY += Math.ceil((paint.descent() - paint.ascent())) / 2;
        // 单删除线
