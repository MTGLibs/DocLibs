...
public class DOCReader extends AbstractReader {
    ...
    public DOCReader(IControl control, String filePath, Uri uri) {
    ...
    private void processDoc() throws Exception {
        InputStream inputStream = StreamUtils.getInputStream(control.getActivity(), uri);
        poiDoc = new HWPFDocument(inputStream);
    ...
    private BackgroundAndFill converFill(HWPFAutoShape shape, OfficeDrawing drawing, int shapeType) {
        ...
        if (shape != null) {
            ...
            if (type == BackgroundAndFill.FILL_SOLID || type == BackgroundAndFill.FILL_BACKGROUND) {
                ...
                com.wxiwei.office.java.awt.Color fillColor = shape.getForegroundColor();
                com.wxiwei.office.java.awt.Color fillbackColor = shape.getFillbackColor();
   ...
   /*         	byte[] data = drawing.getBGPictureData(shape.getBackgroundPictureIdx());
    ...
    private void processWordArtTextbox(EscherContainerRecord escherContainer, WPAutoShape autoShape) {
        ...
        if (text != null && text.length() > 0) {
            ...
            // 字符颜色
            com.wxiwei.office.java.awt.Color color = ShapeKit.getForegroundColor(escherContainer, null, MainConstant.APPLICATION_TYPE_PPT);
            if (color != null) {
    ...
    private boolean processShape(CharacterRun run, IElement leaf, boolean isWrap, int runIndex) {
        if (isWrap) {
            ...
            com.wxiwei.office.fc.hwpf.usermodel.Picture pic = pictureTable.extractPicture(control.getSysKit().getPictureManage().getPicTempPath(),
                    run, false);
