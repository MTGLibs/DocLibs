...
public class DOCXReader extends AbstractReader {
    ...
    public DOCXReader(IControl control, String filePath, Uri uri) {
    ...
    private void openFile() throws Exception {
        ...
        zipPackage = new ZipPackage(inputStream);
        ...
        if (!coreRel.getTargetURI().toString().contains("/word/document")) {
            throw new Exception("Format error");
        }
    ...
    private void processParaAttribute(Element pPr, IAttributeSet attr, int level) {
        ...
        if (temp != null) {
            ...
            if (val != null) {
                float lineUnit = Float.parseFloat(val);
                // 默认行单位是12磅
