    ...
    class SearchThread extends Thread {
        ...
        private void searchContent(File file) throws Exception {
            ...
            if (fileName.endsWith(MainConstant.FILE_TYPE_DOC)
                ...
                reader = new DOCReader(null, file.getAbsolutePath(), null);
            }
            // docx
                ...
                reader = new DOCXReader(null, file.getAbsolutePath(), null);
            }
            //
            else if (fileName.endsWith(MainConstant.FILE_TYPE_TXT)) {
                reader = new TXTReader(null, file.getAbsolutePath(), "GBK", null);
                reader.dispose();
            }
            // xls
            else if (fileName.endsWith(MainConstant.FILE_TYPE_XLS)
                    || fileName.endsWith(MainConstant.FILE_TYPE_XLT)) {
                reader = new XLSReader(control, file.getAbsolutePath(), null);
            }
            // xlsx
                ...
                reader = new XLSXReader(control, file.getAbsolutePath(), null);
            }
            // ppt
            else if (fileName.endsWith(MainConstant.FILE_TYPE_PPT)
                    || fileName.endsWith(MainConstant.FILE_TYPE_POT)) {
                reader = new PPTReader(control, file.getAbsolutePath(), null);
            }
            // pptx
                ...
                reader = new PPTXReader(control, file.getAbsolutePath(), null);
            }
            // PDF document
